...
{
	if (!Window::isInitialized() || inititalized)
	{
		return false;
	}
...
float Input::gamepadAxis(int axis)
{
	if (!isGamepadPresent())
		return 0;
...
bool Input::gamepadButtonPressed(int button)
{
	if (!isGamepadPresent())
		return 0;
	GLFWgamepadstate gamepad_state;
